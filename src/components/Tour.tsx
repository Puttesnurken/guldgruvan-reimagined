  ...
  return (
    <section id="tour" className="py-24 bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto font-extralight text-4xl">
          {shows.slice(0, showAll ? shows.length : 6).map((show, index) => (
            ...
            >
              <div
                className="group relative overflow-hidden rounded-none bg-card border-2 border-primary hover:-translate-y-2 transition-all duration-300 h-full flex flex-col"
                style={{ boxShadow: "none" }}
                onMouseEnter={(e) => (e.currentTarget.style.boxShadow = "8px 8px 0px 0px hsl(var(--primary))")}
              ...
              >
                <div className="bg-secondary/50 p-4 border-b border-border/50">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {!show.available && (
                      <span className="px-2 py-1 bg-destructive/20 text-destructive text-xs font-medium rounded-full whitespace-nowrap">
                        SLUTSÅLT
                      </span>
                    )}
                  </div>
            ...
            <Button onClick={() => setShowAll(!showAll)} variant="outline" className="px-8 py-6 text-lg">
              {showAll ? (
                ...
                <>
                  ...
                  Visa fler datum ({shows.length - 6} till)
                </>
              )}
            </Button>
          </div>
        ...
        <div
          className="mt-12 text-center animate-fade-in opacity-0"
          style={{ animationDelay: "0.8s", animationFillMode: "forwards" }}
        >
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Vi turnerar runt om i Sverige. Håll utkik här för kommande datum nära dig!
          </p>
        </div>
      </div>
    </section>
